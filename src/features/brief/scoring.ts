...
export function calculateReadinessScore(input: BriefInput): ReadinessScore {
  const breakdown: ScoreBreakdown = {
    ...
    offerClarity: scoreTextClarity(input.offerDescription, maxScores.offerClarity),
...
function detectGaps(input: BriefInput, breakdown: ScoreBreakdown) {
  ...
  if (breakdown.goalClarity < 16) {
    gaps.push("Define one primary conversion goal and the exact visitor action.");
  }
  ...
  if (breakdown.audienceClarity < 16) {
    gaps.push("Describe the target audience and buying context in more detail.");
  }
  ...
  if (input.deadline === "asap") {
    gaps.push("Confirm the launch deadline and reduce scope if the timeline is fixed.");
  }

  if (input.budgetRange === "under_1000" || input.budgetRange === "not_sure") {
    gaps.push("Align budget expectations with the desired scope and launch quality.");
  }
